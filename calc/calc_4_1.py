...
class Result:
    ...
    def calculation(self):
        ...
        scnario_description = 'Полное разрушение→ отсутствие мгновенного воспламенения→возможность образования взрывоопасного облака→ отсутствие отсроченного воспламенения → токсическое поражение'
        general = [self.equipment[0], self.equipment[1], self.equipment[7], scenario, scnario_value,
                   scnario_description,
                   mass_all, mass_pf]
        spill_fire = list((0, 0, 0, 0))  # результат расчета пожара
        ...
        dead_man = 0  # т.к. зоны не большие
        injured_man = 0  # т.к. зоны не большие
        ...
        scnario_description = 'Частичная разгерметизация → мгновенное воспламенение→ пожар пролива'
        general = [self.equipment[0], self.equipment[1], self.equipment[7], scenario, scnario_value,
                   scnario_description,
                   mass_pf, mass_pf]
        spill_fire = list(calc_strait_fire.Strait_fire().termal_class_zone(S_spill=self.equipment[4]*PART_NOFULL_DAMAGE, m_sg=M_SG,
                                                                           mol_mass=self.sub[3], t_boiling=self.sub[4],
                                                                           wind_velocity=WIND))  # результат расчета пожара
        explosion = list((0, 0, 0, 0, 0))  # результат расчета взрыва
        jet_fire = list((0, 0))  # результат расчета факела
        ...
        result_damage = calc_damage.Damage(dead_man=dead_man, injured_man=injured_man,
                                           volume_equipment=self.equipment[2], diametr_pipe=0, lenght_pipe=0,
                                           degree_damage=PART_NOFULL_DAMAGE, m_out_spill=0, m_in_spill=mass_all,
        ...
        mass_gas = calc_evaporation.Evaporation(volume_equipment=self.equipment[2]*PART_NOFULL_DAMAGE, degree_filling=1,
                                                spill_square=self.equipment[4]*PART_NOFULL_DAMAGE,
                                                pressure_equipment=self.equipment[5],
                                                temperature_equipment=self.equipment[6], density_liquid=self.sub[2],
                                                molecular_weight=self.sub[3], boiling_temperature_liquid=self.sub[4],
                                                heat_evaporation_liquid=self.sub[5],
        ...
        mass_pf = round(mass_gas * KG_TO_T, 3)
        scnario_description = 'Частичная разгерметизация → отсутствие мгновенного воспламенения→возможность образования взрывоопасного облака→ отсроченное воспламенение → пожар-вспышка'
        general = [self.equipment[0], self.equipment[1], self.equipment[7], scenario, scnario_value,
                   scnario_description,
        ...
        mass_pf = 0
        scnario_description = 'Частичная разгерметизация → отсутствие мгновенного воспламенения → возможность образования взрывоопасного облака → отсуствие отсроченного воспламенения → токсическое поражение'
        general = [self.equipment[0], self.equipment[1], self.equipment[7], scenario, scnario_value,
        ...
        jet_fire = list((0, 0))  # результат расчета факела
        flash_fire = list((0, 0))  # результат расчета вспышки
        toxi = list(calc_toxi.Toxi().LD_PD(spill_square=self.equipment[4]*PART_NOFULL_DAMAGE))  # результат расчета токси
        ball_fire = list((0, 0, 0, 0))  # результат расчета шара
        spill_toxi = list((0,))  # результат расчета токс.пролива
