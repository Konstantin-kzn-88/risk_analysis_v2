...
class Result:
    ...
    def calculation(self):
        ...
        mass_pf = round(PART_IN_FIREBALL * mass_gas * KG_TO_T, 3)
        scnario_description = 'Полное разрушение→ мгновенное воспламенение→ огненный шар'
        ...
        ball_fire = list(
            calc_fireball.Fireball().termal_class_zone(mass_pf * T_TO_KG, 350))  # результат расчета шара
        ...
        toxi = list((0, 0))  # результат расчета токси
        ball_fire = list(
            calc_fireball.Fireball().termal_class_zone(mass_all * T_TO_KG * PART_IN_FIREBALL, 350))  # результат расчета шара
        spill_toxi = list((0,))  # результат расчета токс.пролива
