...
from calc.methods import calc_probit
from calc.methods import _found_nearest_value
...
class Fireball:
    ...
    def fireball_array(self, mass: float, ef: float) -> tuple:
        ...
        while q_term > 1.2:
            ...
            q_term = res[0]
            d_term = res[1]
            probit = calc_probit.Probit().probit_fireball(t_s, q_term)
    ...
    def termal_class_zone(self, mass: float, ef: float) -> list:
        ...
        for CZA in classified_zone_array:
            ind = d_term_array.index(_found_nearest_value.get_nearest_value(d_term_array, CZA))
